...
public class SMGT {
    ...
    /**
     * Finds the critical task in a level (task on critical path with highest
     * priority)
     * For now, we'll use the task with the highest rank as a proxy for critical
     * path
     */
    ...
    /**
     * Finds the task with the largest preference value (lowest preference) in a
     * VM's waiting list
     */
     ...
     * @param vmWaitingLists Map to store VM waiting lists (VM index -> list of
     *                       waiting task IDs)
     */
    public void assignTasksInLevelCorrect(int level, Map<Integer, List<Integer>> vmWaitingLists) {
        ...
        System.out.println("Tasks in level: "
                + tasksInLevel.stream().map(t -> "t" + t).reduce((a, b) -> a + ", " + b).orElse("none"));
        ...
        while (!tasksInLevel.isEmpty()) {
            ...
            for (int j = 0; j < taskPrefList.size(); j++) {
                ...
                // CRITICAL FIX: Check cumulative threshold - VM can't exceed threshold(vm,
                // level)
                ...
                if (vmWaitingList.size() < thresholdValue) {
                    ...
                    if (largestPrefTask != null) {
                        int largestTaskPosition = getTaskPositionInVMPreference(largestPrefTask, vmIndex,
                                vmPreferences);

                        System.out.println(
                                "    VM" + vmIndex + " at threshold (" + thresholdValue + "), checking replacement:");
                        ...
                        if (currentTaskPosition < largestTaskPosition) {
                            ...
                            System.out.println("  ✓ Task t" + currentTask + " replaced t" + largestPrefTask +
                                    " in VM" + vmIndex + " (threshold preserved: " + vmWaitingList.size() + "/"
                                    + thresholdValue + ")");
                            taskAssigned = true;
                            break;
                        } else {
            ...
            if (!taskAssigned) {
                ...
                if (!foundVMWithCapacity) {
                    System.out.println("  ⚠️ WARNING: Task t" + currentTask
                            + " cannot be assigned - all VMs at threshold capacity!");
     ...
     * 
     * @param criticalPath  Set of task IDs that belong to the Critical Path (to be
     *                      excluded)
     * @param cpAssignments Pre-existing assignments of CP tasks from DCP (VM ID ->
     *                      list of task IDs)
                ...
                if (nonCPCount < thresholdValue) {
                    ...
                    if (worstTask != null) {
                        ...
                        if (currentPos < worstPos) {
                            ...
                            taskAssigned = true;
                            System.out
                                    .println("  Task t" + currentTask + " replaced t" + worstTask + " on VM" + vmIndex);
                        }
                    }
                }
            }
     ...
     * 
     * @return Map containing final assignments (VM index -> list of assigned task
     *         IDs)
     */
