...
public class TestValidation {
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("\n✓ Critical Path Found:");
        System.out.println("   CP Tasks: " + dcpResult.criticalPath);
        System.out.println("   CP Length: " + dcpResult.criticalPath.size());
        
        // Verifica che CP contenga un task per livello
        boolean cpLengthOK = dcpResult.criticalPath.size() == taskLevels.size();
        ...
        int expectedNonCP = tasks.size() - dcpResult.criticalPath.size();
        ...
        double minCPTime = calculateMinimumCPExecutionTime(dcpResult.criticalPath, tasks, smgt.getVMs());
            ...
            for (int i = 0; i < vmTasks.size(); i++) {
                for (int j = i + 1; j < vmTasks.size(); j++) {
                    ...
                    if (t1Start != null && t1End != null && t2Start != null && t2End != null) {
                        // Check overlap: tasks overlap if NOT (t1 ends before t2 starts OR t2 ends before t1 starts)
                        boolean noOverlap = (t1End <= t2Start + EPSILON) || (t2End <= t1Start + EPSILON);
                        if (!noOverlap) {
                            overlaps++;
                            System.out.println("   ✗ Overlap on VM" + vmId + ": t" + t1 + 
                                             " [" + String.format("%.2f", t1Start) + "-" + String.format("%.2f", t1End) + "] vs t" + t2 +
        ...
        System.out.println("\n📊 Final Results:");
        System.out.println("   Critical Path: " + dcpResult.criticalPath.size() + " tasks");
    ...
    private static double calculateMinimumCPExecutionTime(Set<Integer> cp, List<task> tasks, List<VM> vms) {
        ...
        double maxCapacity = vms.stream()
            .mapToDouble(vm -> {
                ...
                if (cap <= 0) cap = vm.getCapability("processing");
    ...
    private static double calculateAVU(Map<Integer, List<Integer>> schedule, 
        ...
        for (Map.Entry<Integer, List<Integer>> entry : schedule.entrySet()) {
            ...
            if (capacity <= 0) capacity = vm.getCapability("processing");
